...
def bundle_js():
    ...
    minified_js = safe_jsmin(combined_js)
...
"""
Fix for build.py - Modify the JavaScript minification to properly handle HTML strings

The issue is with the jsmin library minifying HTML content within JavaScript template strings.
When JavaScript files contain HTML template strings, the minification process is incorrectly 
processing the quotes and HTML structure, resulting in malformed HTML like:

<a href="https:<i class=" fas="" fa-external-link-alt="" me-1"="">Open Claude</a>

This fix adds a custom function to properly handle JavaScript files containing HTML.
"""


def safe_jsmin(js_content):
    """
    A custom JavaScript minification function that preserves HTML template strings

    This function:
    1. Extracts template strings (enclosed in backticks) and preserves them
    2. Minifies the JavaScript using the jsmin library
    3. Restores the preserved template strings

    Args:
        js_content (str): The JavaScript content to minify

    Returns:
        str: Minified JavaScript with preserved template strings
    """
    # Find all backtick-enclosed template strings
    template_pattern = re.compile(r'`([\s\S]*?)`', re.MULTILINE)
    templates = template_pattern.findall(js_content)

    # Replace template strings with placeholders
    placeholder_prefix = "___TEMPLATE_STRING_PLACEHOLDER_"
    modified_js = js_content

    for i, template in enumerate(templates):
        placeholder = f"{placeholder_prefix}{i}___"
        modified_js = modified_js.replace(
            f"`{template}`", f'"{placeholder}"', 1)

    # Apply jsmin to the modified content
    minified_js = jsmin.jsmin(modified_js)

    # Restore template strings
    for i, template in enumerate(templates):
        placeholder = f"{placeholder_prefix}{i}___"
        # Preserve line breaks and spaces in the template string
        minified_js = minified_js.replace(
            f'"{placeholder}"', f'`{template}`', 1)

    return minified_js


# Modification for the bundle_js function in build.py
def bundle_js():
    """Bundle all JavaScript files into a single minified file at static/js/main.js"""
    ...
    # Minify JavaScript using our safe minifier that preserves template strings
    print("Minifying JavaScript...")
    # Use our custom function instead of jsmin.jsmin
    minified_js = safe_jsmin(combined_js)
